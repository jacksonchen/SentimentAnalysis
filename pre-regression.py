...
class PreRegression():
  def compile_comments(self, path, app_comments):
    counter = 0
  ...
  def words_table(self, app_comments):
    # Ignored words
  ...
  def words_comments_table(self, total_words, comments_table, words_per_comment):
  ...
  def generate_words_comments(self, words_comments_path, comments_ratings_path, words_per_comment_path, app_comments_path):
    app_comments = pd.DataFrame(columns = ['date_published', 'downloads', 'package', 'reviews', 'version_code', 'version_name'])
    app_comments = self.compile_comments(app_comments_path, app_comments)

    total_words, words_per_comment = self.words_table(app_comments)
    print "\nTotal word count: " + str(len(total_words))

    words_comments, comments_ratings = self.words_comments_table(total_words, app_comments, words_per_comment)
  ...
  def main(self):
    ...
    self.generate_words_comments(words_comments_path, comments_ratings_path, words_per_comment_path, app_comments_path)
