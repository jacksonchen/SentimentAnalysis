...
class Regression():
  def load_data(self, x_path, y_path, words_comments_path, app_comments_path):
  ...
  def predict_true_table(self, y, predicted_df):
  ...
  def consistency(self, predict_true_vals, words_comments, word_vals):
  ...
  def pre_lda(self, consistent_neg_comments, app_comments):
  ...
  def main(self):
    ...
    true_value_path = re.match("true_value_path = '([^']+)'", f[6]).group(1)

    x, y, words_per_comment, app_comments = self.load_data(x_path, y_path, words_comments_path, app_comments_path)
    word_vals, comments_prediction = self.regression(x, y)
    predict_true_vals = self.predict_true_table(y, comments_prediction)
    consistent_neg_comments, inconsistent_comments = self.consistency(predict_true_vals, words_per_comment, word_vals)
    self.pre_lda(consistent_neg_comments, app_comments)

    f = open(true_value_path, 'w')
    f.write(word_vals.to_csv(header = False))
    f.close()
